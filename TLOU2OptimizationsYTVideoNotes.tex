...
Okay the second sentence is daunting! What does it mean? - The parameter cache (sometimes called the interpolator cache or post-transform cache or the VPC - vertex parameter cache in QC GPU's) is a specialized fast-access storage cache in the GPU. Its purpose is to store the outputs of a vertex shader—things like position, normal, texture coordinates, and any other user-defined out variables that must be passed to the rasterizer next. It's a small cache block that stores the steam output from VS transform feedback APIs. If you output too many parameters per vertex, your VPC fills up quickly. It can lead to register spill or write back to the L2 cache for storage and also bound the rasterizes which in-turn limits the number of GPRs available for VS wave usage, also if VPC is filled up it can cause the GPU to stall and kill active waves and decrease occupancy. So GPR allocation can also suffer this way. 

So since we can't do anything else, during this time we can just invoke more VS invocations and have it overlap with PS invocation and basically do the VS mem and ALU ops for free, and maybe even push some ALU from PS to VS as well.
